"use client";

// Homepage "How to Get Started" section: a step indicator + step copy on the
// left, and a colored panel on the right showing that step's device mockup or
// illustration. Renders StatsSection directly above it, both wrapped in one
// pinned block: the wrapper (whatever of StatsSection is still visible above
// it, plus the full walkthrough) pins in place while the user scrolls through
// it — scrolling down/up advances/reverses the active step, with each step's
// image crossfading + parallaxing in direct proportion to scroll position
// (not a fixed-duration transition) so the motion visibly tracks the scroll
// itself — and only releases once every step has been scrolled past, at
// which point the whole wrapper continues scrolling with the rest of the
// page. Clicking a step in the indicator smooth-scrolls straight to it.
import { useRef, useState } from "react";
import Image from "next/image";
import Link from "next/link";
import gsap from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";
import { useGSAP } from "@gsap/react";
import type { HowItWorksContent, HowItWorksStep, StatsContent } from "@/data/mockContent";
import StatsSection from "@/components/ui/StatsSection";
import { useLenis } from "@/components/providers/SmoothScrollProvider";
import { useRequestFuelCardModal } from "@/components/providers/RequestFuelCardModalProvider";

gsap.registerPlugin(ScrollTrigger);

export interface HowItWorksProps {
  content: HowItWorksContent;
  // Omit on pages that don't want the animated stat counters pinned above
  // the walkthrough (e.g. the Agile Flex solutions page) — the homepage
  // always passes this.
  statsContent?: StatsContent;
}

// Panel background per card theme (see HowItWorksStep in mockContent.ts).
const THEME_PANEL: Record<HowItWorksStep["theme"], string> = {
  green: "bg-brand-500",
  lavender: "bg-[#E7E2F7]",
  mint: "bg-[#E4EFE6]",
};

// All 4 steps in mockContent.ts ship a finished image, so all 4 render here.
const VISIBLE_STEPS = 4;

// The 4 step images share one canvas size but not one composition — the fleet
// chat mockup fills its frame edge-to-edge, while the fund-control and
// fuel-anywhere illustrations are small graphics on a lot of empty themed
// canvas, and the reporting screenshot's content runs to its right/bottom
// edge with padding only on the left/top. A single object-fit/position can't
// suit all four, so each step gets its own crop tuned to its source image.
const IMAGE_FIT = [
  "object-cover object-bottom",
  "object-cover object-bottom scale-150",
  "object-cover object-bottom origin-bottom scale-[1.35]",
  "object-cover object-right-bottom origin-bottom-right scale-110",
];

// Scroll distance dedicated to each step while the section is pinned,
// expressed as a fraction of the viewport height so the pace feels the same
// on any screen size.
const VH_PER_STEP = 0.8;

export default function HowItWorks({ content, statsContent }: HowItWorksProps) {
  const lenis = useLenis();
  const { open: openRequestFuelCardModal } = useRequestFuelCardModal();
  const wrapperRef = useRef<HTMLDivElement>(null);
  const sectionRef = useRef<HTMLElement>(null);
  const triggerRef = useRef<ScrollTrigger | null>(null);
  const panelRefs = useRef<(HTMLDivElement | null)[]>([]);
  const [activeIndex, setActiveIndex] = useState(0);
  const steps = content.steps.slice(0, VISIBLE_STEPS);
  const activeStep = steps[activeIndex];

  useGSAP(
    () => {
      if (!sectionRef.current || !wrapperRef.current) return;

      gsap.from(sectionRef.current, {
        y: 12,
        opacity: 0,
        duration: 0.35,
        ease: "power3.out",
        scrollTrigger: { trigger: sectionRef.current, start: "top 85%" },
      });

      // Crossfades + gently parallaxes every step's image by how close scroll
      // progress is to that step's own "slot" — 1 when it's dead centered,
      // fading and drifting out toward its neighbors — so the swap reads as
      // continuous motion driven by scroll, not a click-timed transition.
      // Also drives activeIndex, which the dot indicator and step text read.
      // Step 0's peak sits at raw=0 (scroll start) so it's shown fully loaded
      // — untouched by the effect — and the crossfade only plays out between
      // consecutive steps from there on, not before the first one appears.
      const segments = Math.max(1, steps.length - 1);

      function applyProgress(progress: number) {
        const raw = progress * segments;

        steps.forEach((_, i) => {
          const panelEl = panelRefs.current[i];
          if (!panelEl) return;

          const distance = gsap.utils.clamp(-1, 1, raw - i);
          gsap.set(panelEl, {
            opacity: 1 - Math.abs(distance),
            y: distance * 24,
            scale: 1 - Math.abs(distance) * 0.05,
          });
        });

        setActiveIndex(gsap.utils.clamp(0, steps.length - 1, Math.round(raw)));
      }

      // Pins the WRAPPER (StatsSection + this section together) for the
      // whole walkthrough — timing is still keyed to this section reaching
      // center-viewport (unchanged from a single-section pin), but pinning
      // the wrapper instead of just this section means whatever of
      // StatsSection is still visible above it at that moment freezes right
      // along with it, instead of continuing to scroll away underneath. The
      // page only scrolls past once every step has been stepped through,
      // with the active step tracking scroll position (and direction) the
      // entire way.
      triggerRef.current = ScrollTrigger.create({
        trigger: sectionRef.current,
        start: "center center",
        end: () => `+=${window.innerHeight * VH_PER_STEP * segments}`,
        pin: wrapperRef.current,
        scrub: 1,
        onUpdate: (self) => applyProgress(self.progress),
      });

      applyProgress(triggerRef.current.progress);
    },
    { scope: wrapperRef, dependencies: [steps.length] }
  );

  // Smooth-scrolls to the middle of a step's pinned scroll range. Native
  // scroll events fire throughout, so the crossfade above animates in step
  // with the scroll exactly as it would for a manual scroll.
  function goToStep(index: number) {
    const trigger = triggerRef.current;
    if (!trigger) {
      setActiveIndex(index);
      return;
    }
    const targetProgress = index / Math.max(1, steps.length - 1);
    const targetY = trigger.start + targetProgress * (trigger.end - trigger.start);
    if (lenis) {
      lenis.scrollTo(targetY, { duration: 1 });
    } else {
      window.scrollTo({ top: targetY, behavior: "smooth" });
    }
  }

  return (
    <div ref={wrapperRef}>
      {statsContent && <StatsSection content={statsContent} />}

      <section ref={sectionRef} className="mx-auto max-w-[1536px] px-4 pt-20 text-center sm:px-8">
        <span className="text-xs font-medium uppercase tracking-wide text-orange-500">
          {content.eyebrow}
        </span>
        <h2 className="mt-3 text-2xl font-semibold text-brand-900 sm:text-[36px]">
          {content.title}
        </h2>
        <p className="mt-2 text-sm text-brand-900/50">{content.subtitle}</p>

        <div className="mx-auto mt-16 grid max-w-[1088px] grid-cols-1 items-center gap-12 text-left sm:grid-cols-2 sm:gap-10 lg:gap-16">
          <div className="flex flex-col items-start sm:pr-6 lg:pr-20">
            {/* Step indicator: a pill-shaped track holding the active step as an
                elongated pill and the rest as dots */}
            <div className="inline-flex items-center gap-1.5 rounded-full bg-brand-900/5 p-2">
              {steps.map((step, index) => (
                <button
                  key={step.title}
                  type="button"
                  onClick={() => goToStep(index)}
                  aria-label={`Show step ${index + 1}: ${step.title}`}
                  aria-current={index === activeIndex}
                  className={`h-2 rounded-full transition-all duration-300 ${
                    index === activeIndex ? "w-6 bg-brand-500" : "w-2 bg-brand-900/15"
                  }`}
                />
              ))}
            </div>

            <h3 className="mt-6 text-xl font-semibold text-brand-900 sm:text-2xl">
              {activeStep.title}
            </h3>
            {/* Fixed height (tall enough for the longest step's copy) so the
                CTAs below don't shift up/down as descriptions swap length.
                Taller below xl:, where the text column is narrow enough
                (single-column below sm:, a squeezed half-row from sm: to
                xl:) that it wraps to more lines. */}
            <p className="mt-3 h-28 max-w-xl text-sm leading-relaxed text-brand-900/50 xl:h-12">
              {activeStep.description}
            </p>

            <div className="mt-8 flex flex-col gap-3 sm:gap-4 xl:flex-row">
              <button
                type="button"
                onClick={openRequestFuelCardModal}
                className="flex items-center justify-center gap-2 rounded-full bg-brand-500 px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-brand-600 xl:px-8 xl:py-4"
              >
                {content.primaryCtaText}
                <svg width="14" height="14" viewBox="0 0 12 12" fill="none">
                  <path
                    d="M3 1.5L8.5 6L3 10.5"
                    stroke="currentColor"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                </svg>
              </button>
              <Link
                href="/request"
                className="flex items-center justify-center rounded-full border border-brand-500 px-5 py-3 text-sm font-semibold text-brand-500 transition-colors hover:bg-brand-500/5 xl:px-8 xl:py-4"
              >
                {content.secondaryCtaText}
              </Link>
            </div>
          </div>

          {/* Colored panel, one per step, sized to fill its column up to a
              480px cap (keeps the 544:501 aspect). All steps render at once,
              each crossfading + parallaxing per applyProgress above. */}
          <div className="relative mx-auto aspect-[544/501] w-full max-w-[480px] overflow-hidden rounded-2xl">
            {steps.map((step, index) => (
              <div
                key={step.title}
                ref={(el) => {
                  panelRefs.current[index] = el;
                }}
                className={`absolute inset-0 opacity-0 first:opacity-100 ${THEME_PANEL[step.theme]}`}
              >
                <Image
                  src={step.image.src}
                  alt={step.image.alt}
                  fill
                  sizes="(min-width: 640px) 480px, 100vw"
                  className={IMAGE_FIT[index]}
                />
              </div>
            ))}
          </div>
        </div>
      </section>
    </div>
  );
}
